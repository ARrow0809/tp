...
interface PromptOutputAreaProps {
  ...
  isLoading: boolean; 
  onGenerateImage: (prompt: string, format: OutputFormat | 'ImagePrompt') => void; 
  isGeneratingImage: boolean; 
  canGenerateImage: boolean; 
  onGenerateImagesForAllPrompts: () => void; 
  isGeneratingAllImages: boolean; 
  isSensitiveFilterEnabled: boolean; 
  onToggleSensitiveFilter: () => void; 
}
...
const PromptOutputArea: React.FC<PromptOutputAreaProps> = ({
  ...
  const getButtonText = () => {
    if (selectedTagsCount > 0 && japaneseDescription.trim().length > 0) return `プロンプトを生成 (${selectedTagsCount} タグ + 説明文)`;
    if (selectedTagsCount > 0) return `プロンプトを生成 (${selectedTagsCount} タグ)`;
    if (japaneseDescription.trim().length > 0) return 'プロンプトを生成 (説明文ベース)';
    return 'プロンプトを生成 (データなし)';
  };

  const handleGenerateSingleImage = () => {
    if (currentPromptText && currentFormat !== 'yaml') {
      // The format 'ImagePrompt' (capital I, P) is used for items,
      // but OutputFormat uses 'imagePrompt'. Ensure correct mapping.
      const formatForImageGen: OutputFormat | 'ImagePrompt' = currentFormat === 'imagePrompt' ? 'imagePrompt' : currentFormat;
      onGenerateImage(currentPromptText, formatForImageGen);
    }
  };
    ...
    <div className="p-4 md:p-6 bg-gray-800 rounded-xl shadow-lg space-y-5">
      ...
      <div className="flex items-center justify-start space-x-3 my-3">
        <label htmlFor="sensitiveFilterCheckbox" className="flex items-center text-xs text-gray-300 cursor-pointer select-none">
          ...
          />
          センシティブワードフィルターを有効にする (試験的)
        </label>
      </div>
      
      <button
        onClick={onGenerate}
        disabled={generalLoading || (selectedTagsCount === 0 && !japaneseDescription.trim())} 
        ...
        <div className="mt-4 space-y-3">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              ...
              disabled={!currentPromptText || currentFormat === 'yaml' || generalLoading || !canGenerateImage}
              ...
              title={currentFormat === 'yaml' ? "YAML形式では画像生成できません" : `現在の「${formatLabels[currentFormat]}」プロンプトで画像生成 (${IMAGEN_MODEL_NAME})`}
            >
              {isGeneratingImage && (!generatedPrompts || Object.values(generatedPrompts).filter(p=>p).length <= 1) ? (
                <>
                  <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          ...
          <button
              ...
              title="生成された全てのプロンプト形式で画像を生成します (Stable Diffusion, Midjourney, ImagePrompt)"
            >
              {isGeneratingAllImages ? (
                <>
                  <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
